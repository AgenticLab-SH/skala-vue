...
6. 기준 시각보다 1·2·3시간 빠른 경우와 늦은 경우 여섯 가지를 한 시간 간격으로 비교합니다. 접속과 새로고침 때는 3초 환영 화면을, 추천 요청 중에는 경로 탐색 애니메이션을 보여 줍니다.
...
추천 요청이 빠르게 끝나거나 결과 영역이 화면 아래에 있으면 로딩 애니메이션이 있어도 보이지 않았습니다. 제출 직후 탐색 영역을 화면 가운데로 이동하고, 비교 중 상태를 최소한 짧게 유지했습니다. Lottie Canvas가 준비된 뒤 결과로 바뀌는지 다시 확인했습니다.
...
| dotLottie Web·LottieFiles                      | 접속·새로고침 뒤 3초 환영 화면과 경로·날씨 비교 중 로딩 상태를 Canvas 애니메이션으로 연결했습니다. 애니메이션 원본과 WASM 런타임은 Vite 빌드 결과에서 불러오도록 구성했습니다.               | 매 접속 노출, 3초 자동 닫기, 본문 포커스 복귀, 로딩 중 화면 안 노출, 인스턴스 정리와 `prefers-reduced-motion`을 확인했습니다. |
