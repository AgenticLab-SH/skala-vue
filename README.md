...
- [구현 과정](https://agenticlab-sh.github.io/skala-vue/#/process)
- [수업 실습 기록](https://agenticlab-sh.github.io/skala-vue/#/challenges)
- [레퍼런스](https://agenticlab-sh.github.io/skala-vue/#/reference)
...
### 공개 배포에 OpenWeather 키를 넣을 수 없었습니다

처음에는 GitHub Actions Secret이면 브라우저에서도 키가 보이지 않을 것으로 생각했습니다. Vite의 `VITE_` 변수는 빌드 결과에 포함된다는 점을 확인해 공개 배포는 Open-Meteo를 사용하고, OpenWeather 키는 로컬 `.env.local`에서만 선택하도록 바꿨습니다. production 빌드 뒤 `dist`를 다시 검색해 실제 키가 없음을 확인했습니다. 프론트엔드 환경 변수는 비밀 저장소가 아니라는 점을 배웠습니다.
...
```bash
npm ci
cp .env.example .env.local # OpenWeather를 로컬에서 확인할 때만 값 입력
npm run dev
npm run build
```

개인 키가 없으면 Open-Meteo로 실행됩니다. `main`에 push하면 GitHub Actions가 `npm run build:production`을 실행하고 GitHub Pages에 배포합니다.

## 제출 전 확인

- `npm run lint`, 기본·staging·production 빌드 통과
- 홈·검색·도시 상세·구현 과정·수업 실습·404 경로 확인
