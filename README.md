...
6. 기준 시각보다 1·2·3시간 빠른 경우와 늦은 경우 여섯 가지를 한 시간 간격으로 비교합니다. 접속과 새로고침 때는 2초 환영 화면을, 추천 요청 중에는 최소 1초 동안 투명 유리 레이어의 경로 탐색 애니메이션을 보여 줍니다.
...
추천 요청이 빠르게 끝나거나 결과 영역이 화면 아래에 있으면 로딩 애니메이션이 있어도 보이지 않았습니다. 기존 결과를 지우지 않고 화면 중앙에 고정된 투명 유리 레이어를 별도로 띄웠습니다. 결과가 먼저 준비되어도 최소 1초 동안 유지하고, Lottie Canvas가 나타난 뒤 결과로 돌아오는지 다시 확인했습니다.
...
| dotLottie Web·LottieFiles                      | 접속·새로고침 뒤 2초 환영 화면과 경로·날씨 비교 중 투명 유리 레이어를 Canvas 애니메이션으로 연결했습니다. 애니메이션 원본과 WASM 런타임은 Vite 빌드 결과에서 불러오도록 구성했습니다.           | 매 접속 노출, 2초 자동 닫기, 본문 포커스 복귀, 최소 1초 탐색 레이어, 인스턴스 정리와 `prefers-reduced-motion`을 확인했습니다. |
