...
이 절에는 수업 PDF에서 직접 실습한 Vue 개념과 서비스에 적용한 위치만 정리했습니다. 수업 범위를 넘어 AI의 도움으로 구현한 기술과 그 과정에서 배운 내용은 뒤에서 따로 구분했습니다.
...
- 추천 결과를 포트폴리오나 다른 사람에게 옮기기 전에 포함·제외 정보, 정확도, freshness와 민감성을 확인합니다. 공유용 텍스트는 목적지 도시·활동·대략 이동 시간·날씨만 새로 구성하며 정확한 출발지·장소·시각, 좌표, 검색어, API 정보, 내부 ID·점수와 원문 응답은 사용하지 않습니다. 검토 완료 뒤 복사 버튼을 누를 때만 이 텍스트를 clipboard에 기록하며 Web Share나 네트워크 전송은 하지 않습니다. 검토 뒤 추천 snapshot이 바뀌면 기존 확인을 취소하고 다시 검토해야 합니다.
...
| 기술                  | 수업에서 시작한 부분        | AI를 활용해 구현한 내용                                                    | 구현 후 배운 점                                                                                                 |
| --------------------- | --------------------------- | -------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------- |
| OpenWeather·환경 변수 | 날씨 API 호출과 `.env` 분리 | Actions에서 예보를 받아 키 없는 JSON으로 만든 뒤 정적 화면에서 읽는 구조   | `VITE_` 환경 변수는 브라우저 번들에 포함되므로 비밀 키는 프런트엔드 환경 변수만으로 보호할 수 없다는 점         |
| Promise·비동기 처리   | Promise와 `async`/`await`   | `Promise.allSettled()`로 일부 실패를 허용하고 요청 번호로 최신 실행만 반영 | 비동기 요청은 시작 순서와 완료 순서가 다를 수 있어 최신 요청을 구분하는 상태가 필요하다는 점                    |
| Pinia·Web Storage     | state, getter, action       | 온도 단위·관심 도시·마지막 조건·효과 설정을 `localStorage`에 보존          | Pinia는 실행 중 전역 상태를, `localStorage`는 새로고침 뒤 복원을 맡으며 저장값을 읽을 때도 보정이 필요하다는 점 |

### 교재 밖에서 추가한 기술

| 기술                            | AI의 도움을 받은 부분                               | 구현한 내용                                                              | 구현 후 배운 점                                                                                                          |
| ------------------------------- | --------------------------------------------------- | ------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------ |
| MapLibre GL JS·OpenFreeMap·OSRM | 지도 소스·레이어, 경로 대안과 카메라 이동 구조 조사 | 실제 이동선, 대한민국 전체→목적지 3D 전환, 빠른 경로와 그늘 경로 표시    | 지도 데이터는 source, 표현은 layer로 나뉘며 경로를 비교하려면 하나가 아닌 대안 경로를 요청해야 한다는 점                 |
| SunCalc·그늘 계산               | 태양 위치와 건물 그림자 계산 방법 조사              | 도착 시각의 태양 고도·방위와 건물 높이로 그림자 면과 예상 그늘 비율 계산 | 같은 장소도 시각에 따라 태양각이 달라지고 그 값을 건물 높이와 결합해야 지도 위 그림자 방향과 길이를 구한다는 점          |
| OpenStreetMap Nominatim         | 장소 좌표를 확인하는 방법 조사                      | 활동 장소를 한 번 조회해 정적 좌표로 보관                                | 장소 좌표가 자주 바뀌지 않는다면 실행할 때마다 검색 API를 호출하지 않아도 되어 외부 API 의존성을 줄일 수 있다는 점       |
| 추천 점수·144개 표시 지점       | 활동별 비교 기준과 지도 표시 단위 구체화            | 날씨·장소·이동 시간 점수와 48개 기준 지역의 144개 비교 지점 구성         | 추천 점수는 제가 정한 비교 규칙이므로 객관적 지표처럼 보이지 않게 해야 하며 표시 지점도 기상 레이더와 구분해야 한다는 점 |
| dotLottie·접근성·날씨 모션      | Vue 생명주기와 완료 이벤트 연결 방법 조사           | 웰컴·탐색 상태, 키보드 포커스, 감소된 모션과 지도 날씨 효과 적용         | 애니메이션은 재생만 하는 것이 아니라 load·complete 시점과 인스턴스 정리, reduced motion까지 함께 처리해야 한다는 점      |
