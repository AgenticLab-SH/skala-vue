...
const decisions = [
  ...
  {
    title: '결과를 읽는 순서에 맞춰 화면을 나눴습니다.',
    detail:
      '긴 화면에서 3D 지도를 지나치기 쉬워 출발 조건, 추천 결과, 이동 경로, 시간 변경의 네 구간으로 나눴습니다. 경로 구간에 들어오면 대한민국 전체에서 목적지까지 확대해 3D 건물과 그림자를 먼저 보여 주고, 이후에는 전체보기·확대보기와 직접 조작을 사용할 수 있게 했습니다.',
  },
]
...
const troubleshooting = [
  ...
  {
    problem: '3D 지도와 확대 기능이 있어도 사용자가 조작 가능한 지도인지 알아보기 어려웠습니다.',
    cause: '추천·지도·시간 비교가 한 화면에 이어졌고 지도는 처음부터 같은 배율로 멈춰 있었습니다.',
    fix: '화면을 네 단계로 나누고 이동 경로 구간에 들어올 때 대한민국 전체에서 목적지 3D까지 카메라가 이동하게 했습니다. 전체보기·확대보기 버튼과 드래그 안내를 두고, 모바일은 지도 조작을 켠 동안 한 손 이동과 핀치를 사용한 뒤 페이지 스크롤로 돌아오게 했습니다.',
  },
  {
  ...
  <article class="process-view">
    ...
    <section class="result-section" aria-labelledby="result-title">
      ...
      <div>
        ...
        <ul>
          <li>출발 조건·추천 결과·이동 경로·시간 변경을 네 개의 스크롤 구간으로 나눴습니다.</li>
          <li>18개 도시의 OpenWeather 도착 예보와 활동별 장소를 비교합니다.</li>
          <li>이동 시간 안의 전체 후보와 OSRM 경로를 확인하고 원하는 도시로 바꿀 수 있습니다.</li>
          <li>
            대한민국 전체에서 목적지까지 확대되는 지도에서 3D 건물과 그림자를 확인하고 마우스나
            터치로 직접 움직일 수 있습니다.
          </li>
          <li>앞뒤 출발 시간을 비교해 같은 장소의 더 나은 출발 시각을 다시 선택할 수 있습니다.</li>
